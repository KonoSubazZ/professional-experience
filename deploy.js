...
let src='http://cluster.bwlib.cn'  //上传地址
// let src='http://192.168.10.102:8003/'

let type=2   //状态   1前端  2后端前  
let password ='cq1080'
let username ='admin'
let versionNumber = '1.1.0'  //版本号
let _gitLog //git 打印
...
.then((res,val)=>{
    ...
    if(JSON.parse(res.body).status==200){
        ...
        isExists()
            .then(res=>{
                fs.writeFile(`./text/uploadLog${versionNumber}.text`,`提交时间：${time}，提交版本：${versionNumber}，打印：git:${_gitLog}`,{},res=>{
                    console.log('记录成功')
                })
