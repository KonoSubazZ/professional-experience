<template>
  <div class="face-auth" :class="appType == 'two' ? 'top-35' : ''">
      ...
      <img
        ...
        :class="
          skin == 'child'
            ...
            : 'w-status-light'
        "
        :src="require('@/assets/images/' + skin + '/face_start.png')"
        alt
      />
      <div class="mt-40">
    ...
    <div class="face-loading mt-110-upper" v-if="status == 'loading'">
      <div
        class="w-scan margin-center posi-rela"
        style="width:600px"
        :class="appType == 'two' ? 'two-card-scan' : ''"
      >
        <img
          class="loading"
          :class="appType == 'two' ? 'two-card-scan' : ''"
          src="@/assets/images/child/face_loading.png"
          alt
        />
        <div
          class="posi-scan w-scan-posi"
          :class="appType == 'two' ? 'two-scan-posi' : ''"
        >
          <div id="video" class="video-box" v-if="startStatus == 'loading'">
            ...
            <canvas
              ...
              "
            />
            <div id="overlay" class="video-canvas">
  ...
  methods: {
    async detectExpressions() {
      const result = await faceapi
        .detectSingleFace(this.cam, this.faceapiOptions)
        .withFaceExpressions();
    ...
    pictrue() {
      ...
      if (status == 600) {
        ...
        if (
          ...
          this.faceDraw = canvas.toDataURL("image/png");
          this.faceLogin();
        }
      }
    },
