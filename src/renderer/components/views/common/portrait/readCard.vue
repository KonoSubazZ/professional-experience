...
export default {
  ...
  computed: {
    ...
    list() {
            console.log('this.list',this.$store.state.Borrow.progreeList);
  ...
  methods: {
    ...
    start() {
      ...
      // setTimeout(() => {
      //   this.status = "succes";
      //   this.readSuccess("0CE6FA71");
      // }, 3000);
      // console.log(this.readerCardType, "type");
      let jumpStatus = false;
      let status = this.testReader.readReaderCard(
          this.readerCardType,
          (data) => {
            if (data && !jumpStatus) {
              console.log('readReaderCarddata',data)
              this.status = "succes";
              this.testReader.stopReaderCard();
              jumpStatus = true;
