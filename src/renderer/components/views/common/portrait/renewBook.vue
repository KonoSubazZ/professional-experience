    ...
    <div class="table-info mt-110-table">
        <div>
            <div class="table font-14">
                ...
                <div class="max-table">
                    <div class="tr line h-52" v-for="(item, index) in bookList" :key="index" @click="selectThis(index)">
                        <div class="td flex-one" @click="showMoreInfo(item.positiveTitle)">
                            {{ item.positiveTitle }}
                        </div>
                        <div class="td t-10">{{ item.price }}</div>
                        <div class="td flex-one" @click="showMoreInfo(item.callNumber)">
                            {{ item.callNumber }}
                        </div>
                        <div class="td t-11" @click="showMoreInfo(item.barCode)">
                            {{ item.barCode }}
                        </div>
                        <div class="td t-11" @click="showMoreInfo(item.rackName)">
                            {{ item.borrowAlso }}
                        </div>
                        <div class="td t-10 check-father">
                            <el-checkbox size="small" class="checkbox" :disabled="status == 'success'"
                                v-model="item.select"></el-checkbox>
        ...
        <div class="flex-row flex-center mt-40" v-if="status == 'success'">
            ...
            <button v-if="manageSetting.selfServiceBorrowIsPrint" class="form-yellow-btn font-24" @click="print">
                打印凭条并返回
            </button>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        renewSuccess(res) {
            ...
            this.$store.dispatch("modifyCaption", "续借成功");
            this.getCurrentBorrow();
        },
        /**
         * 获取当前借阅
        ...
        print() {
            ...
            if (status == 606) {
                ...
                this.$store.dispatch("add_to_error_log", "打印续借成功");
                this.reback();
            }
        },
        /**
