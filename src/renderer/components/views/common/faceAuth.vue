<template>
  <div class="face-auth" :class="appType == 'two' ? 'top-35' : ''">
    ...
    <div class="face-loading mt-110-upper" v-if="status == 'loading'">
      <div class="w-scan margin-center posi-rela" :class="appType == 'two' ? 'two-card-scan' : ''">
        ...
        <div class="posi-scan w-scan-posi" :class="appType == 'two' ? 'two-scan-posi' : ''" v-loading="faceLoading"
...
import tracking from "@/assets/tracking/tracking-min";
  ...
  methods: {
    ...
    checkCamera() {
      if (!navigator.mediaDevices || !navigator.mediaDevices.enumerateDevices) {
        ...
        navigator.mediaDevices
              ...
              navigator.mediaDevices
                .getUserMedia({
                  ...
                  video: {
                    deviceId: {
                      exact:
                        list.filter((as) => as.label.indexOf("RGB") > -1)
                          .length != 0
                          ? list.filter((as) => as.label.indexOf("RGB") > -1)[0]
                            .deviceId
                          : list[1].deviceId,
                    },
                  },
                    ...
                    tracker.on("track", function (event) {
                      ...
                      event.data.forEach(function (rect) {
                        ...
                        if (event.data.length) {
                          ...
                          _this.uploadLock && _this.pictrue(video);
                          video.srcObject = null;
                          _this.scanTip = "脸部信息已采集完成"
                        }
                      });
                    });
                  }, 500);
          ...
          .catch((err) => {
            this.$store.dispatch("add_to_error_log", "未获取到摄像头列表");
            // this.$func.toast({
    ...
    faceLogin() {
      ...
      this.$axios
        ...
        .then((res) => {
          console.log('我是res', res.data.photo);
          let data = res.data;
          clearInterval(this.interval);
        ...
        .catch((err) => {
          this.$message.warning(err || '人脸识别失败');
          this.loginStatus = false;
          this.faceLoading = false;
          this.$store.dispatch("add_to_error_log", "人脸认证登录失败");
          this.$store.dispatch(
            "modifyCaption",
            err || "人脸识别失败"
          );
    ...
    getAfterInfo(data) {
      ...
      this.$axios
        ...
        .catch((err) => {
          this.$store.dispatch("add_to_error_log", "获取逾期数据接口调用失败");
    ...
    recertify() {
      this.$store.dispatch(
        "modifyProgreeList",
        this.$func.setPrevFalse(JSON.parse(JSON.stringify(this.oldPressList)))
      );
      this.$router.go(-1);
    },
    /**
