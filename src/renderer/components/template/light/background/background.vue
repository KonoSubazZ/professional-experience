    ...
    <div id="background">
        <div class="back-header bg-light">
            <div class="header-body posi-abso flex-row flex-between align-center">
                ...
                <div class="header-operating color-white text-center">
                    ...
                    <div class="operating-menu pointer pd-20" id="menu-btn" v-if="userInfo && userInfo.name"
                        ...
                        <button class="menu-list font-24 pd-20" id="menu-list" v-if="showMenu">
                            <div v-for="(item, index) in menuList" :key="index" @click="change(item)">
                                {{ item.name }}
                            </div>
                        </button>
                    </div>
        ...
        <div class="caption-user flex-row">
            <div class="user-info flex-row color-333" v-if="userInfo && userInfo.name">
                <el-image class="w-40 mr-10 bg-light bor-rad-round" style="overflow: hidden;width: 6rem;height: 6rem;" :src="userInfo.photo != '' && userInfo.photo != null
                        ? http + '/' + userInfo.photo
                        : require('@/assets/images/dark/user_tag.png')
                    " :fit="'cover'"></el-image>
                <span class="font-24">{{ userInfo ? userInfo.name : "" }}</span>
            </div>
            <captionTime :time="time" :color="'#1DA1F2'" v-if="setting.showTime"></captionTime>
        </div>
...
export default {
    ...
    methods: {
        ...
        goHome() {
            // this.testReader.stopInventoryRFIDReader()
            ...
            // .catch(err => {
            //     try {
            //         this.$children[3].$parent.$children[2].start && this.$children[3].$parent.$children[2].start()
            //     } catch (error) {
            //         console.error(error);
            //     }
