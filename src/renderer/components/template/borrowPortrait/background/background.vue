<template>
  <div id="background">
    <model :models="models" @bool="receiveMessage"></model>
    <div class="back-header bg-light">
      <div class="header-body posi-abso flex-row flex-between align-center">
        ...
        <div class="header-operating color-white text-center">
          ...
          <button class="operating-menu pointer pd-20" id="menu-btn" v-if="userInfo && userInfo.name"
            @blur="showMenu = false" @click="showMenu = !showMenu">
            <div class="font-24">{{ type ? typeList[type] : "逾期" }}</div>
            <div>
              <img class="w-down" src="@/assets/images/lightPortrait/menu-down.png" alt />
            </div>
            <div class="menu-list pd-20" id="menu-list" v-if="showMenu">
              <div style="font-size:1.75rem" v-for="(item, index) in menuList" :key="index" @click="change(item)">
                {{ item.name }}
              </div>
            </div>
          </button>
    ...
    <div class="caption-user flex-row">
      <div class="user-info flex-row color-333" v-if="userInfo && userInfo.name">
        <el-image class="w-40 mr-10 bg-light bor-rad-round" style="overflow: hidden;width: 6rem;height: 6rem;"
          ...
          :fit="'cover'"></el-image>
        <span class="font-24">{{ userInfo ? userInfo.name : "逾期" }}</span>
        
      </div>
      <captionTime :color="'#1DA1F2'" :time="time" v-if="setting.showTime"></captionTime>
    </div>
...
<script>
import captionTime from "@/components/common/portrait/captionTime";
import copyright from "@/components/common/portrait/copyright";
