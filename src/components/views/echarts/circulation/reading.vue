<template>
    <div >
        ...
        <el-dialog title="报表导出格式设置" :visible.sync="createModel" width="90%" top='1vh' :close-on-click-modal='false' >
                <div >
                    <set-view :url="'/manage/unit/circulation/circulationExcel/circulationReadingResForExcelField'" @chooseName='chooseName' @showList='showList'></set-view>   
                    <!-- url 是 -->
                </div>
...
export default {
    ...
    methods: {
      calcTableHeight(val) {
        ...
        lookFor() {
            this.$router.push({
                path: '/circulation/report/readingList',
        ...
        //   读者单位
        getRead() {
            this.ax.get('/manage/unit/report/journalReport/readerUnit',{params:{ presenceStatus:1,platformId :JSON.parse(localStorage.getItem("user")).platform.id,presenceStatus:1}}).then(res => {
