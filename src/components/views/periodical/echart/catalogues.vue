<template>
    <div>
        ...
        >
            <div>
                <set-view
                    :url="'/manage/unit/journal/journalExcel/journalBaseResStatisticsForExcelField'"
...
export default {
    ...
    data() {
        return {bus: this.bus(this),
            list: [
                ...
                { name: '编目批次', id: 'catelogBatchId', type: 'select', relation: 'catelogBatchName', options: [] },
    ...
    methods: {
      calcTableHeight(val) {
        ...
        lookFor() {
            this.$router.push({
                path: '/periodical/echart/cataloguesList',
            ...
            this.ax.get('/manage/meta', { params: { path: 'com.cq1080.library.cluster.bean.entity.Book' } }).then(res => {
                ...
                res.subs.map(it => {
                    if (it.key == 'bookStatus') {
                        this.list[7].options = it.searchOption.map(item => {
