<template>
    <div>
        ...
        >
            <div>
                <set-view
                    :url="'/manage/unit/journal/journalExcel/journalBookingStatisticsForExcelField'"
...
export default {
    ...
    methods: {
      calcTableHeight(val) {
        ...
        lookFor() {
            this.$router.push({
                path: '/periodical/echart/bookList',
        ...
        // 获取类型
        getype(){
            this.ax.get('/manage/unit/report/journalReport/medium',{params:{platformId:JSON.parse(localStorage.getItem("user")).platform.id}})
                .then(res=>{
                    this.list[4].options = res.content.map(it=>{
        ...
        // 获取批次
        getBitch(){
            this.ax.get('/manage/unit/report/journalReport/bookBatch',{params:{platformId:JSON.parse(localStorage.getItem("user")).platform.id}})
                .then(res=>{
            ...
            this.ax.get('/manage/meta', { params: { path: 'com.cq1080.library.cluster.bean.entity.Book' } }).then(res => {
                ...
                res.subs.map(it => {
                    if (it.key == 'bookStatus') {
                        this.list[6].options = it.searchOption.map(item => {
                            return { name: item.name, id: item.key };
                        });
                        // // console.log(this.list[8].options)
                    }
