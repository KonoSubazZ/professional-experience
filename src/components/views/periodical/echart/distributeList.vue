    ...
    methods: {
        exportAll(){
          ...
          this.loading()
            ...
            this.ax.post('/manage/unit/journal/journalExcel/journalBookingReceiveResDisForCreateExcel',{journalBookingAllReq:this.form,format,list})   //需要修改地方 导出
                .then(res=>{
                  //  newTab.location = res
                  window.location = res
                  this.sloading()
        ...
        // 获取
        getList() {
            this.ax.get('/manage/unit/journal/journalExcel/journalBookingReceiveResDis', { params: this.form }).then(res => {    //获取内容   需要修改
                this.data = res
            });
        },
        getForMat(){
        ...
        // 获取字段
        getFied(){
            this.ax.get('/manage/unit/journal/journalExcel/journalBookingReceiveResDisForExcelField')   //需要修改
                .then(res=>{
                    this.lists = res
                })
        },
            ...
            this.meta = {
                ...
                subs: [
                    ...
                    { name: '复本', key: 'copy', displayInList: true },
                      ...
                      { name: '分发馆藏地', key: 'distributionCollectionPlaceName', displayInList: true },

                    { name: '分发总数', key: 'distributionNumber', displayInList: true }, 
