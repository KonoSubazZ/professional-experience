<template>
    <div class="container">
        <List
            :dataUrl="'/manage/unit/circulation/borrowingInformationInquire/borrowingInformationBorrowFalse?borrowType=1'"
            :disableMeta="true" :tableMeta="meta" ref="list" :readerUnitName="readerUnitName" @data='getData' :isHeaderHeigt='30' @search='getSearch'
    ...
    methods: {
        ...
        print() {
            let meta = [
                ...
                { name: '应还日期', key: 'borrowAlso', displayInList: 'true' },
            ]
        ...
        metaList() {
            this.ax.get('/manage/meta', { params: { path: 'com.cq1080.library.cluster.bean.entity.BorrowingInformation' } }).then(res => {
                ...
                res.subs.map(it => {
                    if (it.key == 'createTime' || it.key == 'type' || it.key == 'platformId' || it.key == 'readerId' || it.key == 'borrowBookTime') {
                    ...
                    if (it.key == 'borrowDate' || it.key == 'borrowAlso') {
                        it.searchable = true
                    }
