<template>
    <div >
        ...
        <el-dialog title="报表导出格式设置" :visible.sync="createModel" width="90%" top='1vh' :close-on-click-modal='false' >
                <div >
                    <set-view :url="'/manage/unit/circulation/circulationExcel/circulationDocumentTypeResForExcelField'" @chooseName='chooseName' @showList='showList'></set-view>
                </div>
...
export default {
    ...
    methods: {
      calcTableHeight(val) {
        ...
        lookFor() {
            this.$router.push({
                path: '/circulation/report/categoryList',
            ...
            this.ax.get('/manage/meta', { params: { path: 'com.cq1080.library.cluster.bean.entity.DocumentAppointment' } }).then(res => {
                ...
                res.subs.map(it => {
                    if (it.key == 'documentAppointmentMethod') {
                        this.list[5].options = it.searchOption.map(item => {
