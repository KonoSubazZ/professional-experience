<template>
    <div >
        ...
        <el-dialog title="报表导出格式设置" :visible.sync="createModel" width="90%" top='1vh' :close-on-click-modal='false' >
                <div >
                    <set-view :url="'/manage/unit/circulation/circulationExcel/circulationReaderRankingResForExcelField'" @chooseName='chooseName' @showList='showList'></set-view>
                </div>
...
export default {
    ...
    data() {
        return {bus: this.bus(this),
            list: [
                ...
                { name: '操作人', id: 'managerId',type: 'select', relation: 'managerName', options: []  }
            ],
            form: {},
            height: '',
            arr: [],
    ...
    methods: {
      calcTableHeight(val) {
        ...
        lookFor() {   
            this.$router.push({
                path: '/circulation/report/readRankingList',
