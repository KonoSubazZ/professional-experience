    ...
    <div>
        <div class="container" :style="'height:' + height + 'px'">
            ...
            <div class="padding_20">
                <form-data :row="1" :list="list" :rule="rules" :model="form" @getForm="getForm"></form-data>
            </div>
        </div>
        ...
        <el-dialog title="报表导出格式设置" :visible.sync="createModel" width="90%" top="1vh" :close-on-click-modal="false">
            <div>
                <set-view
                    :url="
                        form.fun == 1
                            ? '/manage/unit/catalog/catalogExcel/bookBaseResForExcelField'
                            : '/manage/unit/catalog/catalogExcel/bookResForExcelField'
                    "
...
export default {
    ...
    data() {
        return {
            bus: this.bus(this),
            rules: {
                fun: [{ required: true, message: '请选择', trigger: 'change' }]
            },
            list: [
                {
                    name: '统计方式',
    ...
    methods: {
        ...
        getType() {
            this.ax
                .get('/manage/unit/report/journalReport/medium', {
        ...
        getCHI() {
            this.ax
                .get('/manage/unit/report/journalReport/language', {
                    params: { platformId: JSON.parse(localStorage.getItem('user')).platform.id }
                })
                .then((res) => {
        ...
        getBitch() {
            this.ax
                .get('/manage/unit/report/journalReport/journalBaseBatch', {
                    params: { platformId: JSON.parse(localStorage.getItem('user')).platform.id }
                })
                .then((res) => {
        ...
        getUnit() {
            this.ax
                .get('/manage/unit/report/journalReport/journalDeliveryUnit', {
                    params: { platformId: JSON.parse(localStorage.getItem('user')).platform.id }
                })
                .then((res) => {
                    // // console.log(res)
        ...
        getCycle() {
            this.ax
                .get('/manage/unit/report/journalReport/journalCycle', {
                    params: { platformId: JSON.parse(localStorage.getItem('user')).platform.id }
                })
                .then((res) => {
