 <template>
    <div class="container">
         <List 
            :dataUrl="'/manage/unit/system/systemSettings/pressLibrary'"
            :disableMeta='true'
            :tableMeta='meta'
            @edit="editUpdate"
            ref="list"
        ...
        <el-dialog title="出版社管理" :visible.sync="createModel" width="30%" :close-on-click-modal='false'  @close='close'>
                <el-form :model="form" :rules="rules" ref="form" label-width="140px" class=" m-b-30">
                        ...
                        <el-form-item label="出版社代码" prop='code' >
                          ...
                          <el-form-item label="出版地" prop='publisherPlace' >
                            <el-input v-model="form.publisherPlace" ></el-input>
                            <!-- <el-checkbox :checked="form.defaultSetting">是否默认批次</el-checkbox> -->
                        </el-form-item>

                          <el-form-item label="地区代码" prop='areaCode' >
                            <el-input v-model="form.areaCode" ></el-input>
                            <!-- <el-checkbox :checked="form.areaCode">是否默认批次</el-checkbox> -->
                        </el-form-item>
                        
                        <!-- <el-form-item label="编目类型" prop='type' >
...
export default {
    ...
    methods: {
        ...
        deleteUpdate(e){
            ...
            this.loading()
            this.ax.delete('/manage/unit/system/systemSettings/pressLibrary?ids='+ids)
                .then(res=>{
                     this.sloading()
                    this.bus.$emit(`flash_/manage/unit/system/systemSettings/pressLibrary`, res);
        ...
        // 保存
         save_mar(){
            ...
            //  this.form.managerId = JSON.parse(localStorage.getItem("user")).info.id
             ...
             this.ax.post('/manage/unit/system/systemSettings/pressLibrary',this.form)
                .then(res=>{
                    this.sloading()
        ...
        metaList(){
           this.ax.get('/manage/meta',{params:{path:'com.cq1080.library.cluster.bean.entity.PressLibrary'}})
            .then(res=>{
                res.delete='DELETE'
                res.subs.map(it=>{
