<template>
    <div>
        ...
        <div class="margin_center ">
            <el-upload class="m-b-20" ref="upload" drag :action="src" :auto-upload='false' :data='data'
                :file-list="fileList" :before-upload='isText' :on-success="handleAvatarSuccess" :headers='headers'>
                <i class="el-icon-upload"></i>
                <div class="el-upload__text">将文件拖到此处，或<em>点击上传</em>,仅支持TXT文件</div>
            </el-upload>
            <!-- title有的话是馆藏调拨功能 -->
            <div class="display_flex start between " v-if="title">
                <!-- 
                ...
                <div class="display_flex w-80"></div>
                <div class="cursor color_par w-20" @click="upload">开始调拨</div>
            </div>
            <div class="display_flex start between " v-else>
...
export default {
    ...
    methods: {
        // 开始上传
        upload() {
            if (!this.collectionPlaceId&&this.title) return this.$message.error('请选择馆藏地')
