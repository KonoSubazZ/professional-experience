<template>
    <div class="m-b-50">
        <quill-editor
            ...
            @input="sendMsg"
        ></quill-editor>
        <el-upload class="upload-demo" action="/other/uploadFile" :headers='headers' :data="{ type }"  :on-success='upScuccess' :before-upload="startUpload"
...
const toolbarOptions = [
    ...
    [{ size: ['small', false, '16px', 'large', 'huge'] }], // 字体大小-----[{ size: ['small', false, 'large', 'huge'] }]
...
export default {
    ...
    methods: {
        getUrl(){
            this.ax.get('/manage/unit/general/projectAddress')
                .then(res=>{
                    console.log(res)
                    this.urls = res
                })
